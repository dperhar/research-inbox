...
export default function Onboarding({ onComplete }: OnboardingProps) {
  ...
  if (step === "welcome") {
    return (
      <div className="h-screen flex flex-col items-center justify-center px-8 bg-[var(--bg)] text-[var(--text)]">
        <div className="text-3xl mb-3">🔗</div>
        <h1 className="text-lg font-bold mb-1">Research Inbox</h1>
        <p className="text-[13px] text-[var(--text-secondary)] text-center mb-6 max-w-[260px]">
        ...
        <div className="w-full max-w-[260px] space-y-4 mb-7">
          <div className="flex items-center gap-3">
            <kbd className="px-2 py-1 rounded bg-[var(--bg-secondary)] border border-[var(--border)] text-[11px] font-mono font-bold shrink-0">⇧⌘S</kbd>
            <p className="text-[12px] leading-snug">Saves selected text + offers screenshot</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded bg-[var(--bg-secondary)] border border-[var(--border)] flex items-center justify-center shrink-0">
  ...
  if (step === "accessibility") {
    return (
      <div className="h-screen flex flex-col items-center justify-center px-8 bg-[var(--bg)] text-[var(--text)]">
        ...
        <p className="text-sm text-[var(--text-secondary)] text-center mb-5 leading-relaxed max-w-[280px]">
          RI needs these to capture text from other apps with ⇧⌘S.
        </p>

        <div className="w-full max-w-[280px] bg-[var(--bg-secondary)] rounded-lg p-4 mb-3 space-y-1.5">
          <p className="text-xs font-semibold text-[var(--accent)]">1. Accessibility</p>
          <p className="text-[11px] text-[var(--text-secondary)]">Open → find Research Inbox → toggle ON</p>
        </div>
        <div className="w-full max-w-[280px] bg-[var(--bg-secondary)] rounded-lg p-4 mb-5 space-y-1.5">
          <p className="text-xs font-semibold text-[var(--accent)]">2. Input Monitoring</p>
          <p className="text-[11px] text-[var(--text-secondary)]">Open → click <strong>+</strong> → select <strong>Research Inbox</strong> from Applications → toggle ON</p>
        </div>
        ...
        <div className="flex flex-col items-center gap-2 w-full max-w-[260px]">
          ...
          <button
            ...
            className="w-full py-2.5 bg-[var(--bg-secondary)] border border-[var(--border)] text-[var(--text)] text-sm font-medium rounded-lg hover:bg-[var(--border)] transition-colors"
          >
          ...
          <button
            onClick={onComplete}
            className="text-[11px] text-[var(--text-secondary)] hover:text-[var(--text)] transition-colors"
          >
            Skip for now
          </button>
        </div>
  ...
  return (
    <div className="h-screen flex flex-col items-center justify-center px-8 bg-[var(--bg)] text-[var(--text)]">
      ...
      <p className="text-sm text-[var(--text-secondary)] text-center mb-6 leading-relaxed max-w-[280px]">
        Select text in any app and press <strong>⇧⌘S</strong> to capture it.
        The overlay will appear at the top of your screen.
      </p>

      <div className="bg-[var(--bg-secondary)] rounded-lg px-4 py-3 mb-6 text-center">
        <kbd className="text-lg font-mono font-bold text-[var(--accent)]">⇧⌘S</kbd>
        <p className="text-[11px] text-[var(--text-secondary)] mt-1">Your capture hotkey</p>
      </div>
