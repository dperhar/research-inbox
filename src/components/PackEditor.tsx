  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2 border-b border-[var(--border)] flex items-center gap-2">
        <button onClick={() => setEditingPack(null)} className="text-[var(--text-secondary)] hover:text-[var(--text)]">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
        <input
          ...
          className="w-full px-2.5 py-1.5 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-md text-sm outline-none focus:border-[var(--accent)]"
        />
        <textarea
          value={description}
          ...
          rows={2}
          className="w-full px-2.5 py-1.5 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-md text-xs outline-none focus:border-[var(--accent)] resize-none"
        />

        {/* Items */}
        <div>
          <div className="text-[11px] text-[var(--text-secondary)] mb-1 font-medium">
            Evidence ({packItems.length} items)
          </div>
          <div className="space-y-1">
            {packItems.map((item, idx) => (
              <div key={item.id} className="flex items-start gap-1 p-1.5 bg-[var(--bg-secondary)] rounded border border-[var(--border)]">
                <div className="flex flex-col gap-0.5">
                  <button onClick={() => moveItem(idx, -1)} className="text-[var(--text-secondary)] hover:text-[var(--text)] text-[10px]">▲</button>
                  <button onClick={() => moveItem(idx, 1)} className="text-[var(--text-secondary)] hover:text-[var(--text)] text-[10px]">▼</button>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[10px] text-[var(--text-secondary)]">{item.source_app} · {new Date(item.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
                  <div className="text-xs truncate selectable">{item.content.slice(0, 80)}...</div>
                </div>
                <button onClick={() => removeItem(item.id)} className="text-[var(--text-secondary)] hover:text-red-500 text-xs">&times;</button>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Format selector */}
        <div>
          <div className="text-[11px] text-[var(--text-secondary)] mb-1 font-medium">{t("export_format")}</div>
          <div className="flex gap-1">
            {(["markdown", "claude", "chatgpt", "cursor"] as ExportFormat[]).map((f) => (
              <button
                key={f}
                onClick={() => setFormat(f)}
                className={`px-2 py-1 rounded text-[11px] font-medium transition-colors ${format === f ? "bg-[var(--accent)] text-white" : "bg-[var(--bg-secondary)] text-[var(--text-secondary)] hover:bg-[var(--border)]"}`}
              >
                {f}
              </button>
            ))}
      ...
      <div className="px-3 py-2 border-t border-[var(--border)] flex gap-2">
        ...
        <button onClick={handleSaveAsFile} className="px-2 py-1.5 bg-[var(--bg-secondary)] text-[var(--text)] text-xs font-medium rounded-md border border-[var(--border)] hover:bg-[var(--border)] transition-colors">
          {t("save_as_file")}
        </button>
        <button onClick={handleSave} className="px-2 py-1.5 bg-[var(--bg-secondary)] text-[var(--text)] text-xs font-medium rounded-md border border-[var(--border)] hover:bg-[var(--border)] transition-colors">
          {t("save_pack")}
        </button>
      </div>
    </div>
